...
{
    if (a.number == 0) return b.Normalized();
    if (b.number == 0) return a.Normalized();

    if (a.exponent > b.exponent + 10) return a.Normalized();
    if (b.exponent > a.exponent + 10) return b.Normalized();

    int diff = a.exponent - b.exponent;
    if (diff > 0)
        return new BigNumber(a.number + b.number / Math.Pow(10, diff), a.exponent).Normalized();
    else
        return new BigNumber(b.number + a.number / Math.Pow(10, -diff), b.exponent).Normalized();
}
    ...
    public static BigNumber operator *(BigNumber a, BigNumber b)
    {
        ...
        return new BigNumber(newnumber, newExponent).Normalized();
    }
...
/// <summary>
/// Compares two BigNumbers as if they are whole integers. Ignores floating point drift.
/// How to use:
/// a > b	CompareWhole(a, b) > 0
/// a == b	CompareWhole(a, b) == 0
/// a < b	CompareWhole(a, b) < 0
/// a >= b	CompareWhole(a, b) >= 0 ✅
/// a <= b	CompareWhole(a, b) <= 0
/// </summary>
public static int CompareWhole(BigNumber a, BigNumber b)
{
    // Skip normalization: we're just comparing full integer values
    // Scale the smaller one up to match exponents for whole number comparison
    if (a.exponent > b.exponent)
    {
        int diff = a.exponent - b.exponent;
        double bScaled = b.number / Math.Pow(10, diff);
        return a.number.CompareTo(bScaled);
    }
    else if (a.exponent < b.exponent)
    {
        int diff = b.exponent - a.exponent;
        double aScaled = a.number / Math.Pow(10, diff);
        return aScaled.CompareTo(b.number);
    }
    else
    {
...
private static void Normalize(ref double number, ref int exponent)
{
    ...
    // Snap to 8 digits of precision (adjust if needed)
    number = Math.Round(number, 7, MidpointRounding.AwayFromZero);

    // Special case: if number rounds to 10.0, renormalize one step up
    if (number >= 10)
    {
        number /= 10;
        exponent++;
    ...
    public static BigNumber operator -(BigNumber a, BigNumber b)
{
    ...
    if (diff > 0)
        ...
        return new BigNumber(a.number / Math.Pow(10, -diff) - b.number, b.exponent).Normalized();
}
    public static BigNumber Parse(string input)
{
    ...
    {
        return new BigNumber(numValue * multiplier, 0).Normalized();
    }
