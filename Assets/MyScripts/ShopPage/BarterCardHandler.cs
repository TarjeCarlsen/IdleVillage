...
public class BarterCardHandler : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        print($"price index = {chosenPriceIndex} reward index = {chosenRewardIndex}");
        priceValue = GetRandomValue(chosenPriceIndex);
        rewardValue = GetRandomValue(chosenRewardIndex);
        print($"price value = {priceValue} reward value = {rewardValue}");

        chosenMerchantIndex = GetRandomMerchant();
        print("chosen merchant = " + chosenMerchantIndex);

        level = barterManager.merchantInfos[(Merchants)chosenMerchantIndex].merchantLevel;
        print("merchant level = " + barterManager.merchantInfos[(Merchants)chosenMerchantIndex].merchantLevel);

        priceAmount = GetRandomAmount(level);
        print("random amount gotten = " + priceAmount);
    ...
    private AlphabeticNotation CalculateReward()
    {
        ...
        randomFactor = UnityEngine.Random.Range(0.5f, 1.5f);
        tradeValue = priceValue + rewardValue * (float)rewardAmount;

        level  = barterManager.merchantInfos[(Merchants)chosenMerchantIndex].merchantLevel;
        xpReward = baseXp 
              + (Mathf.Pow(tradeValue, valueFactor) * Mathf.Pow(level + 1, -levelFactor))
              * randomFactor;
    ...
    private void UpdateUI()
    {
        ...
        price_txt.text = priceAmount.ToStringSmart(0);
        reward_txt.text = rewardAmount.ToStringSmart(0);
        ...
        xpIncreaseProgressBar.SetProgress((xpReward+barterManager.merchantInfos[(Merchants)chosenMerchantIndex].merchantXp) / 
                                                   barterManager.merchantInfos[(Merchants)chosenMerchantIndex].requiredXp);
        // currentXp_txt.text = $"{barterManager.merchantInfos[(Merchants)chosenMerchantIndex].merchantXp:F1}";
        // requiredXp_txt.text = $"{barterManager.merchantInfos[(Merchants)chosenMerchantIndex].requiredXp:F1}";
