...
// IF you want any fraction, e.g. 0.999, to be "fixed", then uncomment the following line.
// #define FIXANYFRACTIONS
...
namespace LargeNumbers
{
    ...
    public struct AlphabeticNotation : IEquatable<AlphabeticNotation>
    {
        ...
        public static Func<double, int, string> ToStringFunction;
        ...
        {
            if ( string.IsNullOrEmpty ( alphabeticNotationString ) )
            ...
            else // spaces == 1
            {
                ...
                if ( componenets.Length != 2 )
                { alphabeticNotation = default; return false; }   // Safety check.
            }
...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        {
            if ( string.IsNullOrEmpty ( alphabeticName ) )
            ...
            if ( alphabeticName.Length > 7 )
            { magnitude = 0; return false; }
            if ( alphabeticName.Length == 1 )
            {
                for ( int i = 0, count = _standardNames.Length; i < count; ++i )
                    if ( alphabeticName == _standardNames [ i ] )
                    { magnitude = fraction ? -i : i; return true; }
            }

            {
                ...
                {
                    if ( alphabeticName [ i ] < 'a' || alphabeticName [ i ] > Alphabet [ alphabetCount - 1 ] )
                    { magnitude = 0; return false; }
        ...
        /// To implement your own "ToString", you can either add a ToStringFunction Func, or create an extension method.
        /// Type-Conversion (changing a numeric value to a string) will pretty much guarantee the production of garbage.
        /// </summary>
        public override string ToString ( )
        {
            if ( ToStringFunction != null )
                return ToStringFunction ( coefficient, magnitude );
